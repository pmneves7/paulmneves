(function () {
  ...
  function keydown(e) { const cm=initState(); if(state().activeTab!=="colormap" || !cm.selectedKey || e.target.matches("input, textarea")) return; const d={ArrowLeft:[-1,0],ArrowRight:[1,0],ArrowUp:[0,-1],ArrowDown:[0,1]}[e.key]; if(!d) return; const refs=(cm.selectedKey==="c1"||cm.selectedKey==="c2")?cm.colorCal:cm.cal,p=refs[cm.selectedKey]; if(!p) return; const step=e.shiftKey?5:.5;p.x=Math.round((p.x+d[0]*step)*2)/2;p.y=Math.round((p.y+d[1]*step)*2)/2;if(els.linkOrigin.checked&&(cm.selectedKey==="x1"||cm.selectedKey==="y1")){const other=cm.selectedKey==="x1"?"y1":"x1";cm.cal[other]={x:p.x,y:p.y};}state().cursor={x:p.x,y:p.y};state().pointerInside=true;invalidate(cm);e.preventDefault();hooks.refreshAll(); }
  function drawZoomRect(ctx,r,color,sx,sy,k){if(!validRect(r))return;ctx.save();ctx.strokeStyle=color;ctx.lineWidth=2;ctx.setLineDash([6,4]);ctx.strokeRect((r.x-sx)*k,(r.y-sy)*k,r.w*k,r.h*k);ctx.setLineDash([]);for(const h of Object.values(handles(r))){ctx.beginPath();ctx.arc((h.x-sx)*k,(h.y-sy)*k,4,0,Math.PI*2);ctx.fillStyle="#fff";ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=1.5;ctx.stroke();}ctx.restore();}
  function drawZoom(ctx,sx,sy,k,marker){const cm=initState();if(validRect(cm.plot)){const nx=Math.max(1,Math.round(val("nx")||1)),ny=Math.max(1,Math.round(val("ny")||1)),dx=cm.plot.w/nx,dy=cm.plot.h/ny,viewW=ctx.canvas.width/k,viewH=ctx.canvas.height/k;ctx.save();ctx.strokeStyle="rgba(255,255,255,.72)";ctx.lineWidth=.75;const xStart=Math.max(0,Math.ceil((sx-cm.plot.x)/dx)),xEnd=Math.min(nx,Math.floor((sx+viewW-cm.plot.x)/dx));for(let i=xStart;i<=xEnd;i++){const x=(cm.plot.x+i*dx-sx)*k;ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,ctx.canvas.height);ctx.stroke();}const yStart=Math.max(0,Math.ceil((sy-cm.plot.y)/dy)),yEnd=Math.min(ny,Math.floor((sy+viewH-cm.plot.y)/dy));for(let i=yStart;i<=yEnd;i++){const y=(cm.plot.y+i*dy-sy)*k;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(ctx.canvas.width,y);ctx.stroke();}ctx.restore();}drawZoomRect(ctx,cm.plot,"#19a974",sx,sy,k);drawZoomRect(ctx,cm.bar,"#d68100",sx,sy,k);if(cm.drag)drawZoomRect(ctx,rect(cm.drag.start,cm.drag.current),"#fff",sx,sy,k);["y1","y2","x1","x2"].forEach(key=>{const p=cm.cal[key];if(p)marker(p,key[0]==="y"?"#1f4163":"#a25d12",null,cm.selectedKey===key);});["c1","c2"].forEach(key=>{const p=cm.colorCal[key];if(p)marker(p,"#7a3d8a",null,cm.selectedKey===key);});}
  function dataAtCursor(p) { const cm=initState(); return ready(cm) ? dataXY(p,cm) : null; }
  function selectedText() { const cm=initState(),refs=(cm.selectedKey==="c1"||cm.selectedKey==="c2")?cm.colorCal:cm.cal,p=cm.selectedKey&&refs[cm.selectedKey],labels={x1:"X₁",x2:"X₂",y1:"Y₁",y2:"Y₂",c1:"C₁",c2:"C₂"},half=v=>Math.round(v*2)/2;return p?`Selected ${labels[cm.selectedKey]} @ (${half(p.x)}, ${half(p.y)}) — use arrow keys`:""; }
  ...
  function status(_state, statusEl){if(state().activeTab!=="colormap")return false;const m=state().mode;statusEl.textContent=m==="cm-run"?"Click Convert, or use the Convert button again after changing settings.":labels[m]?`Click${m==="cm-plot"||m==="cm-bar"?" and drag":""} to set the ${labels[m]}.`:"Choose a calibration or rectangle tool.";return true;}
  ...
  window.DigitizerColormap = {
    ...
    init(h) {
      ...
      els.linkOrigin.addEventListener("change", () => {
        if (!els.linkOrigin.checked) return;
        const cm = initState();
        if (cm.cal.y1) cm.cal.x1 = { ...cm.cal.y1 };
        else if (cm.cal.x1) cm.cal.y1 = { ...cm.cal.x1 };
        if (state().activeTab === "colormap" && state().mode === "cm-x1" && cm.cal.x1 && cm.cal.y1) {
          cm.selectedKey = "x1";
          state().mode = cm.cal.y2 ? "cm-x2" : "cm-y2";
          state().modeByTab.colormap = state().mode;
        }
        invalidate(cm);
        hooks.refreshAll();
      });
